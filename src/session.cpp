...
void APISession::createNormalizedASTContext(const std::string& key){
...
void APISession::processFile(std::string fileName, std::unique_ptr<clang::tooling::FixedCompilationDatabase> m_compDB) {
...
ASTNormalizedContext* APISession::getContext(const std::string& fileName) const {
...
ASTNormalizedContext* APISession::getContext(llvm::StringRef fileName) const {
    auto it = m_contexts.find(fileName);
    if (it != m_contexts.end())  return it->second.get();
    else {

...
void processHeaderPair(const std::string& project1,
    ...
    const ASTNormalizedContext* context1 = session->getContext(file1);
    const ASTNormalizedContext* context2 = session->getContext(file2);
    ...
    nlohmann::json diffResult = diffTrees(
        context1->getRootNodes(), 
        context2->getRootNodes(), 
        context1->getTree(), 
        context2->getTree(),
        context1->excludeNodes,
        context2->excludeNodes
    );
    ...
    if (!diffResult.empty()) {
        ...
        report_generator(outputFile, trimmed_path, htmlReportFile, jsonReportFile, generate_json);
    } else { 
        try {
            std::vector<json> emptyData;
            generate_html_report(emptyData, htmlReportFile);
